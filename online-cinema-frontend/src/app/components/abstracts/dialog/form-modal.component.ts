...
@Component({
  ...
  <form [formGroup]="data.formGroup" (ngSubmit)="onSubmit()">
    ...
    <div fxLayout="row wrap">
      <div fxFlex="100" fxLayout="column" style="padding: 5px;">
        <button mat-button>Подтвердить</button>
      </div>
    </div>
  </form>
  <div fxLayout="row wrap">
...
export class FormModalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.abstractForm.createForm(this.componentFactoryResolver, this.data.formGroup, this.data.formItems);
  }
  ...
  onSubmit() {
    if (this.data.formGroup.valid) {
      this.openConfirmDialog(this.data.title, "Вы хотите подтвердить?").subscribe((result: any) => {
        if (result) {
